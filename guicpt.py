...
class Application:
        ...
        for index in range(max_frames):
            ...
            if last_good_kpts is not None:
                for i in range(len(frame_p3ds)):
                    if np.all(frame_p3ds[i] == -1):
                        frame_p3ds[i] = last_good_kpts[i]
            ...
            self.draw_3d_skeleton(frame_p3ds, skeleton_connections)
    ...
    def draw_3d_skeleton(self, kpts, skeleton_connections):
        ...
        if not np.all(left_shoulder == -1) and not np.all(right_shoulder == -1) and \
           not np.all(left_ear == -1) and not np.all(right_ear == -1):
            ...
            head_center = (left_ear + right_ear) / 2.0
            neck_length = np.linalg.norm(head_center - shoulder_midpoint)
            
            MAX_NECK_LENGTH = 5.0 

            if neck_length < MAX_NECK_LENGTH:
                self.ax_3d.plot(
        ...
        VIEW_RANGE_XY = 100
        VIEW_RANGE_Z = 30
        self.ax_3d.set_xlim3d(-VIEW_RANGE_XY, VIEW_RANGE_XY)
        self.ax_3d.set_ylim3d(-VIEW_RANGE_XY, VIEW_RANGE_XY)
        self.ax_3d.set_zlim3d(-VIEW_RANGE_Z, VIEW_RANGE_Z)
    ...
    def get_skeleton_definition(self):
        ...
        skeleton_connections = torso + head + arm_l + arm_r + leg_l + leg_r + foot_l + foot_r
    ...
    def update_frame_display(self, value):
        ...
        self.draw_3d_skeleton(kpts_3d_frame, skeleton_connections)
        self.canvas_3d.draw()
    ...
    def save(self):
        import glob
        path = './images/'
        image_files = sorted(glob.glob(os.path.join(path, 'cam0_*.png')))
        if not image_files:
